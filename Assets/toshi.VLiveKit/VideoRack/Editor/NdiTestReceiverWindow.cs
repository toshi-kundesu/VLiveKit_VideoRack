...
namespace VLiveKit.VideoRack.Editor
{
    public sealed class NdiTestReceiverWindow : EditorWindow
    {
        ...
        private Component liveReceiver;
        private MethodInfo receiverUpdateMethod;
        ...
        private void OnEditorUpdate()
        {
            ...
            try
            {
                InvokeReceiverUpdate(liveReceiver);
            }
            catch (Exception exception)
            {
                var message = "Window NDI receiver update failed: " + exception.Message;
                StopWindowReceiver();
        ...
        private void DrawCreateSection()
        {
            using (new EditorGUILayout.VerticalScope(panelStyle))
            {
                ...
                {
                    if (GUILayout.Button("Create Scene Receiver", primaryButtonStyle, GUILayout.Height(30)))
                        CreateTestReceiver();
                }
            }
        }

        private void DrawWindowReceiverSection()
        {
            using (new EditorGUILayout.VerticalScope(panelStyle))
            {
                DrawSectionTitle("Window Receiver");
                var isRunning = liveRoot != null;
                DrawStatus(isRunning
                    ? "Receiving into this editor window. No scene object is saved."
                    : "Starts a temporary editor-only receiver without entering Play Mode.");
                ...
                using (new EditorGUILayout.HorizontalScope())
                {
                    using (new EditorGUI.DisabledScope(isRunning || string.IsNullOrWhiteSpace(ndiName) || FindType("Klak.Ndi.NdiReceiver") == null))
                    {
                        if (GUILayout.Button("Start Window Receiver", primaryButtonStyle, GUILayout.Height(28)))
                            StartWindowReceiver();
                    }
        ...
        private void StartWindowReceiver()
        {
            StopWindowReceiver();

            liveTexture = CreateRuntimeRenderTexture("VLiveKit NDI Window Receiver");
            liveRoot = new GameObject("VLiveKit NDI Window Receiver")
            {
                hideFlags = HideFlags.HideAndDontSave
            };

            liveReceiver = AddAndConfigureNdiReceiver(liveRoot, liveTexture);
            statusMessage = liveReceiver != null
                ? "Window NDI receiver started."
                : "KlakNDI receiver is not available.";
            ShowNotification(new GUIContent("Window NDI receiver started"));
        }

        private void StopWindowReceiver()
        {
            liveReceiver = null;
        ...
        private Component AddAndConfigureNdiReceiver(GameObject target, RenderTexture texture)
        {
            ...
            InvokePrivate(receiver, "OnValidate");
            return receiver;
        }
        ...
        private void InvokeReceiverUpdate(Component receiver)
        {
            if (receiver == null)
                return;

            if (receiverUpdateMethod == null)
                receiverUpdateMethod = receiver.GetType().GetMethod("Update", BindingFlags.Instance | BindingFlags.NonPublic);
            receiverUpdateMethod?.Invoke(receiver, null);
        }
        ...
        private static void DrawPreviewTexture(Texture texture)
        {
            ...
            var rect = GUILayoutUtility.GetAspectRect(aspect, GUILayout.MinHeight(120), GUILayout.MaxHeight(260));
